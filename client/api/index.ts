...
export const getPosts = async (cookies: string | undefined): Promise<IResponseGetPosts> => {
  const response = await fetch("http://localhost:3001/post/getPosts", {
    method: "GET",
    credentials: "include",
...
export const deletePost = async (title: string) => {
  const response = await fetch(`http://localhost:3001/post/delete/${title}`, {
    method: "DELETE",
    credentials: "include",
    headers: { "Content-Type": "application/json" },
  });
...
export const resetPassword = async (data: { password: string; resetToken: string | string[] }): Promise<IResponseResetPassword> => {
  const response = await fetch(`http://localhost:3001/auth/resetpassword/${data.resetToken}`, {
    method: "PUT",
    credentials: "include",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(data),
...
export const getLoggedIn = async (): Promise<IResponseLoggedIn> => {
  const response = await fetch(`http://localhost:3001/auth/loggedIn`, {
    method: "GET",
    credentials: "include",
  });
  const _data = await response.json();
